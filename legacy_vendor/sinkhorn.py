...
def sinkhorn_log_actQuant(matrix,
                 order=8,
                 clip_min=1e-3,
                 clip_max=1e3,
                 eps=1e-6,
    ...
    log_mu1 = torch.zeros(m.shape[1], dtype=dtype, device=dev)
    # log_mu2 = torch.zeros(m.shape[0], 1, dtype=dtype, device=dev)
    ################## log_mu2: scalar for rows (single value)
    log_mu2 = torch.zeros(1, dtype=dtype, device=dev)

    # Known-good candidates for the step k=0
    cur0          = m
    ib0           = imbalance(cur0)
    imb_min       = torch.minimum(imb_min, ib0)
    mu1_star      = log_mu1.exp().clone()   # shape (num_cols,)
    mu2_star      = log_mu2.exp().clone()   # scalar (shape (1,))
    ...
    for _ in range(order):
        ...
        better    = (ib <= imb_min).to(dtype)   # 1 if new best
        imb_min   = torch.min(imb_min, ib)
        ################## mu1_star shape matches log_mu1.exp()
        mu1_star  = torch.where(better.bool(), log_mu1.exp(), mu1_star)
        mu2_star  = torch.where(better.bool(), log_mu2.exp(), mu2_star)
        ...
        ########### NEW: aggregate std_r into a single representative scalar ---
        # Option chosen: mean of per-row stds. Alternatives: max() or median().
        std_r_scalar = std_r.mean()   # scalar representing average row std
        sal_row = (std_r_scalar / tgt_small).clamp(0.7, 2.0).log()  # scalar

        log_mu1 = (log_mu1 + (sal_col * g)).clip(-.3, 10.)
        log_mu2 = (log_mu2 + (sal_row * g)).clip(-.3, 10.)

    # final scaled matrix and the recorded best scaling vectors
    # mu1_star: (n_cols,), mu2_star: scalar (shape (1,))
    scaled = m / mu1_star / mu2_star
    return scaled, mu1_star, mu2_star

##################################################################################################################
##################################################################################################################
###################################### TO TRY THE ONLY_COLUMN CODE ######################################
def sinkhorn_log_actQuant_Column_Based(matrix,
                 order=8,
                 clip_min=1e-3,
                 clip_max=1e3,
                 eps=1e-6,
    ...
    tgt_small = m.std(0).clamp(clip_min, clip_max).min() + eps
