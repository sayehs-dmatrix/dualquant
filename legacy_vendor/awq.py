...
def compute_awq_scale(weights, activations, fake_quant_fn, num_alphas=20):
    """
    Conventional AWQ scale computation (Lin et al., 2023).

    Searches over alpha in [0,1] for the per-input-channel scale
        s = clip(mu_x^alpha / mu_w^(1-alpha))
    that minimises output reconstruction error when weights are scaled
    up by s, fake-quantised, then scaled back down.

    Args:
        weights:       (out_features, in_features) float32, on any device
        activations:   (N, seq_len, in_features)  float32 — captured from
                       calibration forward passes; must be on same device
        fake_quant_fn: callable(W_scaled) -> W_q  — should use the actual
                       target format and block size (e.g. wraps pure_qunatize)
        num_alphas:    number of alpha grid points (default 20)

    Returns:
        scale: (in_features,) best per-channel scale, same dtype as weights
    """
    dtype  = weights.dtype
    device = weights.device
    W      = weights.float()
    X      = activations.to(device).float()

    # Per-input-channel importance statistics
    mu_x = X.abs().mean(dim=[0, 1])          # (in_features,)
    mu_w = W.abs().mean(dim=0)               # (in_features,)

    # Flatten activations for matmul: (N*seq, in_features)
    X_flat = X.reshape(-1, X.shape[-1])
    gt     = X_flat @ W.T                    # (N*seq, out_features)

    best_scale = torch.ones(W.shape[1], device=device)
    best_error = float('inf')

    for alpha in np.linspace(0, 1, num_alphas):
        # AWQ paper eq. (4): s = (sx^alpha) / (sw^(1-alpha))
        scale = (mu_x.pow(alpha) / (mu_w.pow(1 - alpha) + 1e-4)).clamp(min=1e-4)
        scale = scale / (scale.max() * scale.min()).sqrt()   # geometric normalise

        # Scale input channels up, fake-quant with the actual target format, scale back
        W_scaled = W * scale.unsqueeze(0)
        W_q      = fake_quant_fn(W_scaled.to(dtype)).float() / scale.unsqueeze(0)

        pred  = X_flat @ W_q.T
        error = (pred - gt).square().mean().item()

        if error < best_error:
            best_error = error
            best_scale = scale

    return best_scale.to(dtype)
